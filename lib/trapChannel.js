...
// A button reads as a live counter "badge" rather than a wall of embed text -
// matches how other moderation bots surface a running count. It's clickable:
// index.js's interactionCreate handler responds to this exact customId with
// an ephemeral info+stats panel (see buildTrapInfoPanel/buildTrapStatsPanel).
function buildKickCounterRow(count) {
...
async function sendTrapMessage(channel, guildId, text, count) {
  const row = buildKickCounterRow(count);
  try {
